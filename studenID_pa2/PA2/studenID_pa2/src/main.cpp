...
int calculateMaxSubset(vector< vector<char> >& maxCut, vector< vector<int> >& maxSubset,
                       const vector<int>& chords, const int& N, const int& smallestLen, int i,
                       int j) {
    ...
    if ((chords[i] < i || chords[i] > j) && (chords[j] < i || chords[j] > j)) {
        maxSubset[i][j] =
            calculateMaxSubset(maxCut, maxSubset, chords, N, smallestLen, i + 1, j - 1);
        maxCut[i][j] = 'N';  // None
    }
    // case 2
    // both two chords containing the internal ends are inside of the internal
        ...
        if (chords[i] > chords[j]) {
            if (calculateMaxSubset(maxCut, maxSubset, chords, N, smallestLen, i, j - 1) >=
                calculateMaxSubset(maxCut, maxSubset, chords, N, smallestLen, i + 1, j)) {
                if (maxSubset[i][j - 1] >=
                    calculateMaxSubset(maxCut, maxSubset, chords, N, smallestLen, i + 1, j - 1)) {
            ...
            maxSubset[i][j] =
                calculateMaxSubset(maxCut, maxSubset, chords, N, smallestLen, chords[j], j) +
                calculateMaxSubset(maxCut, maxSubset, chords, N, smallestLen, i, chords[i]) +
                calculateMaxSubset(maxCut, maxSubset, chords, N, smallestLen, chords[i], chords[j]);
            maxCut[i][j] = 'B';  // Both
        }
    }
    // case 3
    // the internal ends forms a chord
        ...
        maxSubset[i][j] =
            calculateMaxSubset(maxCut, maxSubset, chords, N, smallestLen, i + 1, j - 1) + 1;
        maxCut[i][j] = 'S';  // Self
    }
    // case 4
    // one of two chords containing the internal ends is inside of the internal
        ...
        if (calculateMaxSubset(maxCut, maxSubset, chords, N, smallestLen, i + 1, j) >=
            calculateMaxSubset(maxCut, maxSubset, chords, N, smallestLen, i + 1, j - 1)) {
            maxSubset[i][j] = maxSubset[i + 1][j];
            maxCut[i][j] = 'L';  // Left
        } else {
...
int main(int argc, char* argv[]) {
    ...
    //     }
    // }
    calculateMaxSubset(maxCut, maxSubset, chords, N, smallestLen, 0, N - 1);
